...
module.exports = {
  projects: [
    ...
    },
    {
      ...
      clearMocks: true,
      restoreMocks: true,
      // The 20s-vs-5s CI budget this project needs (hosted runner, single worker, heavy
      // full-screen renders — see the rationale above `jest.setTimeout` in
      // `test/setup/native.ts`) cannot be set here: Jest 29 validates each `projects[]` entry
      // against a schema that has no `testTimeout` key, so it logs an "Unknown option" warning
      // and drops it — and even a normalized value would never reach the scheduler, because
      // `groupOptions()` reads `testTimeout` off the run's one shared *global* config, not off
      // any individual project. `test/jest-config-projects.test.ts` guards against this coming
      // back.
    },
  ],
